...
def show_graph(request):
    ...
    #influences1 = [{'Category': 'Influence', 'viewsUpv': int(i.views) + int((i.vote_levels.filter(influence_id=i.pk, level=1).count()) * 2), 'Status': i.status.capitalize()} for i in Influence.objects.filter(views__gt=0)]
