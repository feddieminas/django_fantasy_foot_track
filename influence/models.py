...
The main first class Influence has all its field attributes plus the owner that creates it 
...
likeability.users_vote field. The other main field of the Likeability class is the influence 
...
class Likeability(models.Model):
    ...
    class Meta:
        unique_together = ('influence', 'users_vote',) # one row, one user enters / modifies choice to one category card    
